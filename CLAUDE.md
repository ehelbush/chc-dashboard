...
- **Root-cause fix for the mid-week lockouts (2026-07, commit 36276a0):** `schwab_sync.py`'s `refresh_access_token()` now keeps the ORIGINAL refresh token on every access-token refresh instead of persisting the rotated one Schwab returns. Persisting the rotation across two token stores (local `schwab_tokens.json` and the `SCHWAB_TOKENS` secret) made them drift and get 401'd mid-week, which forced browser re-auths far more often than the 7-day cap requires. Schwab's refresh token is valid the full 7 days and is reusable read-only (confirmed against the reference model, see the deferred-porting note in Open Questions). The refresh token now changes only at browser re-auth, so both stores always match. Do NOT reintroduce `save_tokens` overwriting `refresh_token` with the rotated value.
