...
- **The refresh token is hard-capped at 7 days and does NOT extend on use.** The daily workflow only refreshes the short-lived *access* token *within* that 7-day window (preserving the original `refresh_expires_at`). It cannot extend the refresh token.
- **ACTUAL root cause of the mid-week "dashboard down" outages (2026-07-03, commit 37fea3c): the `SCHWAB_TOKENS` secret was stored as multi-line JSON, which CI corrupted.** `save_tokens` wrote pretty-printed (`indent=2`) JSON, and the workflow reconstructs the file with `echo "$SCHWAB_TOKENS" > schwab_tokens.json`. Multi-line secrets get mangled in that round-trip for some token values; since tokens are random base64 the corruption was intermittent, which is why it looked like a random recurring curse. Proven by test: the same token that 401'd in CI stored multi-line synced cleanly stored single-line. **Fix:** `save_tokens` in both `schwab_sync.py` and `schwab_auth.py` now write SINGLE-LINE JSON (`separators=(",", ":")`, no indent) so the secret round-trips cleanly. **Do NOT reintroduce `indent=` in `save_tokens`.**
  - *Diagnosis shortcut if a similar "needs reauth despite a valid token" recurs:* if a direct refresh with the LOCAL `schwab_tokens.json` refresh token succeeds but CI 401s, CI's secret copy is corrupt/stale, not the token. Realign: `python3 -c "import json;t=json.load(open('schwab_tokens.json'));print(json.dumps(t,separators=(',',':')),end='')" | gh secret set SCHWAB_TOKENS`, then re-sync locally and push.
  - *Resilience (same commit):* `main()` now only degrades the cache to `needs_reauth` when the refresh token is GENUINELY expired; a transient/desync failure preserves the last-good cache instead of committing a "down" banner.
- **Secondary hardening (2026-07-02, commit 36276a0):** `refresh_access_token()` also stopped persisting the rotated refresh token Schwab returns (keeps the ORIGINAL: `new_tokens["refresh_token"] = tokens["refresh_token"]`). Schwab's refresh token is valid the full 7 days and reusable read-only (Schwab actually returns the SAME token on refresh, verified), matching the reference model. Belt-and-suspenders now that single-line JSON fixed the real problem; still, do NOT reintroduce rotated-token persistence.
...
- The CI secret push (`gh secret set SCHWAB_TOKENS`) needs a valid `GH_PAT` secret. If the PAT expires, CI can't update the token secret and logs "Could not update tokens secret" (masked as success by a `|| echo` fallback). The same `GH_PAT` powers Vercel's save_params API — update it in both places (GitHub Actions secret and Vercel env var).
...
- **Deferred to a future project (tabled 2026-07): port the re-auth to an always-on service.** After the 2026-07 rotation fix, the only remaining manual step is the roughly weekly browser MFA login, which is unavoidable for personal Schwab apps (no longer-lived grant exists). The reference model runs the entire flow as an always-on AWS service (CDK + Docker + gunicorn/nginx, with a single AWS Secrets Manager token store) at `~/projects/cherryhead/2026.03.13 CHC Model Claude /Stock-Updater` (see `src/stock_common/src/stock_common/schwab_account.py`). Porting to that pattern would remove the laptop and LaunchAgent dependency entirely. Not needed now that the mid-week lockouts are fixed; revisit as its own project if the weekly local login becomes a burden.
